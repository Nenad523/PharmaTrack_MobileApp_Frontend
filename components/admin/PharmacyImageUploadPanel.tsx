...
export function PharmacyImageUploadPanel({ pharmacy, isBusy, onUploadImage }: Props) {
  ...
  return (
    <View className="rounded-2xl border border-slate-200 bg-white p-4">
      <Text className="text-base font-bold text-slate-900">Slika apoteke</Text>
      <Text className="mt-0.5 text-xs text-slate-500 mb-3">
        Fotografija fasade ili ulaza u apoteku.
      </Text>

      {!pharmacy ? (
        <View className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
          <Text className="text-sm text-slate-500">Izaberi apoteku prije upload-a slike.</Text>
        </View>
      ) : (
        <>
